...
<template>
  <NuxtLayout name="courses">
    <EmptyList
      v-if="coursesListStore.list.length === 0"
      icon="IconCourses"
      :text="$t('course.empty_list')"
    />
    <div v-else>
      <div class="container-card">
        <CourseCard
          v-for="data in coursesListStore.list"
          :key="data.ID"
          :course="data"
        />
      </div>
      <div
        ...
        ref="observerTarget"
        v-if="
          coursesListStore.pagination.hasNextPage &&
          !coursesListStore.pagination.loading
        "
      >
        <LoadingCourseCard v-for="(_, index) in 1" :key="index" />
      </div>
    </div>
  </NuxtLayout>
</template>
