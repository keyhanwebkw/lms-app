...
export const VerifyPhone = defineStore("VerifyPhone", {
  ...
  actions: {
    ...
    async resendCode() {
      ...
      const { status, error, data }: ResultApiType = await POST_USER_SEND_OTP({
        mobile: this.mobile,
    ...
    async checkLengthValue() {
      Object.keys(this.dataModel).forEach((key) => {
        ...
        this.dataModel[typedKey] = Number(
          this.dataModel[typedKey].toString().slice(0, 1)
        );
      });
    },
